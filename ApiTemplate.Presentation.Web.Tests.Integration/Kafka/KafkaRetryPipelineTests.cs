...
namespace ApiTemplate.Presentation.Web.Tests.Integration.Kafka
{
    ...
    public class KafkaRetryPipelineTests : IAsyncLifetime
    {
        ...
        public KafkaRetryPipelineTests(TestsWebApplicationFactory factory)
        {
            Factory = factory;
            ...
            while (DateTime.UtcNow < timeout && !retryMessageFound)
            {
                try
                {
                    ...
                    if (consumeResult != null && consumeResult.Message != null && consumeResult.Message.Value != null)
                    {
                        ...
                        if (messageValue.Contains(testEvent.Id.ToString()) || messageValue.Contains("TestId"))
                        {
        ...
        // ============================================================================
        // Note: ComprehensiveRetryFlow_ShouldRouteToCorrectTopicsWithProperHeadersAndTimestamps
        // has been moved to KafkaRetryPipelineIsolatedTests to avoid race conditions
        ...
        public async Task ConcurrentExecutionEdgeCases_ShouldHandleMultipleScenariosSimultaneously()
        {
            ...
            var event10s = new KafkaRetryTestEvent(3001, "retry10s@example.com", "Retry10s", "Test");
            var event1m = new KafkaRetryTestEvent(3002, "retry1m@example.com", "Retry1m", "Test");
            var event5m = new KafkaRetryTestEvent(3003, "retry5m@example.com", "Retry5m", "Test");
            var eventDlq = new KafkaRetryTestEvent(3004, "dlq@example.com", "DLQ", "Test");
            var eventConcurrent = new KafkaRetryTestEvent(3005, "concurrent@example.com", "Concurrent", "Test");
        ...
        // Note: RetryConsumer_ShouldProcessMessagesWithPastRetryAtImmediately
        // has been moved to KafkaRetryPipelineIsolatedTests to avoid race conditions
        ...
        {
            // Arrange
            var testEvent1 = new KafkaRetryTestEvent(6001, "offset1@example.com", "Offset", "Test1");
            var testEvent2 = new KafkaRetryTestEvent(6002, "offset2@example.com", "Offset", "Test2");
            var mainTopic = "test.apitemplate.kafka_retry_test_event";
            var retryTopic = $"{mainTopic}.retry.10s";
            var dlqTopic = $"{mainTopic}.dlq";
            ...
            var testEvent3 = new KafkaRetryTestEvent(6003, "offset3@example.com", "Offset", "Test3");
        ...
        // ============================================================================
        // Note: RetryDisabledAndPartitionKey_ShouldPreserveKeyAndCommitWithoutRetry
        // has been moved to KafkaRetryPipelineIsolatedTests to avoid race conditions

        // ============================================================================
        // RETRYAT FUTURE WAITING AND HEADER EXTRACTION TESTS
        // ============================================================================
        // Note: RetryAtFutureWaitingAndHeaderExtraction_ShouldWaitAndExtractHeadersCorrectly
        // has been moved to KafkaRetryPipelineIsolatedTests to avoid race conditions
    }
}