...
namespace ApiTemplate.EventBus.Kafka
{
    internal sealed class SubscriptionsProcessor : ISubscriptionsProcessor, IDisposable
    {
        ...
        {
            _logger.Debug("Gracefully shutting down Kafka subscriptions processor");
        ...
        private async Task ProcessTypedSubscriptionsAsync(
            ...
            CancellationToken cancellationToken)
        {
            if (TryDeserializeIntegrationEvent(eventMetadata.TopicName ?? string.Empty, message, eventMetadata.PartitionKey, out var eventType, out var integrationEvent) && integrationEvent != null && eventType != null)
            {
                using var scope = _serviceScopeFactory.CreateScope();
        ...
        private int GetRetryCountFromHeaders(EventMetadata eventMetadata)
        {
            ...
            if (eventMetadata.Headers != null)
            {
                try
                {
                    var headers = eventMetadata.Headers as Headers;
                    if (headers != null)
                    {
                        var retryCountHeader = headers.FirstOrDefault(h => h.Key == "X-Retry-Count");
            ...
            try
            {
                ...
                if (nextRetryCount > _retryConfiguration.MaxRetryAttempts)
                {
                    ...
                    var dlqEnvelope = RetryMessageEnvelope.Create(
                        originalTopic: eventMetadata.TopicName ?? string.Empty,
                ...
                else
                {
                    ...
                    var retryTopic = _retryConfiguration.GetRetryTopicName(eventMetadata.TopicName ?? string.Empty, retryLevel);
