...
namespace ApiTemplate.EventBus.Kafka
{
    internal sealed class KafkaClient : IEventBus, IDisposable
    {
        ...
        public void Dispose()
        {
            ...
            _logger.Debug("Shutting down Kafka client");
            ...
            try
            {
                ...
                _ = Task.Run(async () =>
                {
                    try
                    {
                        ...
                        _logger.Debug("Starting retry consumers for topic {Topic}", topic);
                        ...
                        _logger.Debug("Successfully started retry consumers for topic {Topic}", topic);
                    }
                    catch (Exception ex)
                    {
        ...
        internal async Task ProcessEventAsync(
            ...
            CancellationToken cancellationToken)
        {
            _logger.Debug("processing event, id: {eventMessageId}", eventMetadata.PartitionKey);

            var subscriptions = _subscriptionManager.GetHandlersForEvent(eventMetadata.TopicName ?? string.Empty);
        ...
        private HealthCheckResult CheckConsumerHealth()
        {
            try
            {
                // Check Kafka connectivity by getting metadata
                // With auto-creation enabled, topics don't exist until first use, so we only check connectivity
                var metadataResult = _adminClient.GetMetadata(TimeSpan.FromSeconds(10));

                // If we can get metadata, Kafka is reachable
                // Topics will be auto-created on first publish/subscribe, so we don't check for their existence
                return HealthCheckResult.Healthy("Kafka broker is reachable. Topics will be auto-created on first use.");
            }
            catch (Exception ex)
            {
                return HealthCheckResult.Unhealthy("Unable to connect to Kafka broker", ex);
            }
        }
            ...
            try
            {
                ...
                while (retryCount <= maxRetries)
                {
                    try
                    {
                        ...
                        {
                            // Store offset locally - Kafka will auto-commit stored offsets periodically
                            // (with EnableAutoCommit=true). This avoids heavy network calls on every message.
                            consumer.StoreOffset(consumeResult);
                            // No manual Commit() - let Kafka batch commits automatically for better performance
                        }
                ...
                if (consumeResult != null && _subscriptionManager.HasSubscriptionsForEvent(consumeResult.Topic))
                {
                    ...
                    try
                    {
                        using (_metrics.Measure.Timer.Time(_processAsyncTimer, metricTags))
                        {
                            ...
                            _logger.Debug("event consumed, id: {consumeResultKey}", consumeResult.Message.Key);
                        }
                    }
                    catch (Exception ex)
                    {
        ...
        {
            var wholeTopic = typeof(T).GetCustomAttribute<KafkaTopicAttribute>();
            if (wholeTopic?.Name == null)
            {
                _logger.Warning("KafkaTopicAttribute not found or Name is null for type {Type}", typeof(T).Name);
                return;
            }
            ...
            CancellationToken cancellationToken)
        {
            ...
            _logger.Debug("Closing Kafka consumer for topic {Topic} due to shutdown", topicName);
        }
            ...
            bool isFirstMessage)
        {
            ...
            try
            {
                ...
                if (isFirstMessage)
                {
                    ...
                    _logger.Debug("Kafka consumer for topic {Topic} successfully processed first message", topicName);
                }
            }
        ...
        private void CloseConsumer(IConsumer<string, string>? consumer, string topicName)
        {
            if (consumer != null)
            {
                ...
                {
                    consumer.Dispose();
                    _logger.Debug("Successfully closed Kafka consumer for topic {Topic}", topicName);
                }
