...
namespace ApiTemplate.EventBus.Kafka.Retry
{
    ...
    internal sealed class DlqConsumer : IDlqConsumer, IDisposable
    {
        ...
        public Task StartAsync(string dlqTopic, CancellationToken cancellationToken)
        {
            ...
            _logger.Debug("Started DLQ consumer for topic {DlqTopic}", dlqTopic);
        ...
        {
            _logger.Debug("DLQ consumer started for {DlqTopic}", dlqTopic);
            ...
            try
            {
                while (!cancellationToken.IsCancellationRequested)
                {
                    try
                    {
                        if (_consumer == null)
                        {
                            _logger.Error("Consumer is null, cannot process DLQ messages");
                            break;
                        }
                    ...
                    {
                        _logger.Debug("DLQ consumer for {DlqTopic} cancelled", dlqTopic);
                        break;
                    }
                    catch (Exception ex)
                    {
            ...
            {
                _logger.Debug("DLQ consumer for {DlqTopic} stopped", dlqTopic);
            }
        }
        ...
        private async Task ProcessDlqMessageAsync(ConsumeResult<string, string> consumeResult, string dlqTopic)
        {
            try
            {
                if (_consumer == null)
                {
                    _logger.Error("Consumer is null, cannot process DLQ message");
                    return;
                }
                ...
                if (envelope == null)
                {
                    ...
                    // Store and commit offset immediately for DLQ (similar to retry topics)
