...
namespace ApiTemplate.Infrastructure.EventBus.Retry
{
    ...
    internal sealed class RetryPipelineService : IRetryPipelineService, IDisposable
    {
        ...
        public async Task StartRetryConsumersAsync(string mainTopic, CancellationToken cancellationToken = default)
        {
            ...
            {
                _logger.Information("Retry pipeline is disabled. Skipping retry consumers for topic {MainTopic}", mainTopic);
                return;
            }

            _logger.Information("Starting retry consumers for main topic {MainTopic}", mainTopic);
            ...
            foreach (var retryLevel in retryLevels)
            {
                ...
                var task = Task.Run(
                    async () =>
                    {
                        ...
                        {
                            _logger.Information("Retry consumer for {RetryTopic} stopped due to cancellation", retryTopic);
                        }
                        catch (Exception ex)
                        {
                ...
                _logger.Information("Started retry consumer for {RetryTopic}", retryTopic);
            }
        }
        ...
        public async Task StartDlqConsumerAsync(string mainTopic, CancellationToken cancellationToken = default)
        {
            if (!_retryConfiguration.IsRetryEnabled)
            {
                _logger.Information("Retry pipeline is disabled. Skipping DLQ consumer for topic {MainTopic}", mainTopic);
                return;
            }
            ...
            _ = Task.Run(
                async () =>
                {
                    ...
                    {
                        _logger.Information("DLQ consumer for {DlqTopic} stopped due to cancellation", dlqTopic);
                    }
                    catch (Exception ex)
                    {
        ...
        public void Dispose()
        {
            ...
            lock (_disposeLock)
            {
                ...
                try
                {
                    _logger.Information("Disposing RetryPipelineService");
